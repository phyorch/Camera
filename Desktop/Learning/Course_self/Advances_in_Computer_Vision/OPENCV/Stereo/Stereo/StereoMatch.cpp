...
*/
int StereoMatch::init(int imgWidth, int imgHeight, string xmlFilePath)
{
...
//void StereoMatch::easyinit(int imgWidth, int imgHeight)
//{
//	m_frameWidth = imgWidth;
//	m_frameHeight = imgHeight;
//	m_numberOfDisparies = 0;
//	m_Calib_Mat_Remap_X_R = 
//	m_Calib_Mat_Remap_Y_R = 
//}
...
* 参数 : xmlFilePath	[in]	双目定标结果数据文件
*/
int StereoMatch::loadCalibData(string xmlFilePath)
{
...
int StereoMatch::bmMatch(cv::Mat& frameLeft, cv::Mat& frameRight, cv::Mat& disparity, cv::Mat& imageLeft, cv::Mat& imageRight)
{
	...
	{
		if (init(frameLeft.cols, frameLeft.rows, "C:/Users/Phyorch/Desktop/Learning/Course_self/Advances_in_Computer_Vision/OPENCV/Stereo/calibration_parameters.xml") == 0)	//执行类初始化
		{
			return 0;
		}
	}
...
* 参数 : filename		[in]	文件路径
*/
void StereoMatch::savePointClouds(cv::Mat& pointClouds, string filename)
{
