...
*/
int StereoCalib::initCornerData(int nImages, cv::Size imageSize, cv::Size boardSize, float squareWidth)
{
	corner_datas.nImages = nImages;
	corner_datas.imageSize = imageSize;
	corner_datas.boardSize = boardSize;
	corner_datas.nPoints = nImages * boardSize.width * boardSize.height;
	corner_datas.nPointsPerImage = boardSize.width * boardSize.height;
	corner_datas.objectPoints.resize(nImages, vector<cv::Point3f>(corner_datas.nPointsPerImage, cv::Point3f(0, 0, 0)));
	corner_datas.imagePoints1.resize(nImages, vector<cv::Point2f>(corner_datas.nPointsPerImage, cv::Point2f(0, 0)));
	corner_datas.imagePoints2.resize(nImages, vector<cv::Point2f>(corner_datas.nPointsPerImage, cv::Point2f(0, 0)));
	...
	for (i = 0; i < nImages; i++)
	{
		...
		for (j = 0; j < boardSize.height; j++)
			for (k = 0; k < boardSize.width; k++)
				corner_datas.objectPoints[i][n++] = cv::Point3f(j*squareWidth, k*squareWidth, 0);
	}
...
*/
int StereoCalib::loadCornerData(string filename, CornerDatas& cornerDatas)
{
...
*/
int StereoCalib::saveCornerData(string filename, const CornerDatas& cornerDatas)
{
...
int StereoCalib::detectCorners(cv::Mat& img1, cv::Mat& img2, CornerDatas& cornerDatas, int imageCount)
{
	...
	cornerDatas.imagePoints1[imageCount] = corners1;
	cornerDatas.imagePoints2[imageCount] = corners2;
...
*/
int StereoCalib::loadCameraParams(string filename, CameraParams& cameraParams)
{
	cv::FileStorage fs(filename, cv::FileStorage::READ);
	if (fs.isOpened())
	{
...
* 参数 : filename		[in]	参数文件路径/文件名
*/
int StereoCalib::saveCameraParams(const CameraParams& cameraParams, string filename /* = "cameraParams.yml" */)
{
...
int StereoCalib::calibrateStereoCamera(CornerDatas& cornerDatas, StereoParams& stereoParams, bool cameraUncalibrated /* = false */)
{
	if (cameraUncalibrated)
	{
		...
		*/
		...
		cout << stereoParams.cameraParams1.distortionCoefficients;
		cv::calibrateCamera(
			cornerDatas.objectPoints,
		...
		*	保存单目定标结果至本地
		*/
		saveCameraParams(stereoParams.cameraParams1, (m_workDir + "cameraParams_left.xml").c_str());
		saveCameraParams(stereoParams.cameraParams2, (m_workDir + "cameraParams_right.xml").c_str());
	}
...
* 参数 : err			[out]	双目标定误差
*/
int StereoCalib::getStereoCalibrateError(CornerDatas& cornerDatas, StereoParams& stereoParams, double& err)
{
	// 利用对极线约束检查立体校正效果
...
*/
int StereoCalib::saveCalibrationDatas(string filename, RECTIFYMETHOD method, CornerDatas& cornerDatas, StereoParams& stereoParams, RemapMatrixs& remapMatrixs)
{
	cv::FileStorage fs(filename, cv::FileStorage::WRITE);
