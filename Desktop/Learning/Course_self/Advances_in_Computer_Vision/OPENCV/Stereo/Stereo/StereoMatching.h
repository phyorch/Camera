...
public:
	...
	*/
	int init(int imgWidth, int imgHeight, string xmlFilePath);
	...
	* 参数 : filename		[in]	文件路径
	*/
	void savePointClouds(cv::Mat& pointClouds, string filename);
	...
	*/
	...
	cv::Ptr<cv::StereoSGBM> m_SGBM = cv::StereoSGBM::create();
...
private:
	...
	* 参数 : xmlFilePath	[in]	双目定标结果数据文件
	*/
	int loadCalibData(string xmlFilePath);
