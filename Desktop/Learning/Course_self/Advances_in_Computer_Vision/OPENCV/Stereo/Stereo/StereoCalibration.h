...
public:
	StereoCalib(void);
	~StereoCalib(void);
	CornerDatas corner_datas;
	CameraParams camera_params;
	StereoParams stereo_params;
	RemapMatrixs remap_matrixs;
	///***
	//*	棋盘角点数据 结构体
	//*/
	//struct CornerDatas
	...
	*/
	...
	void setWorkDir(string workDir) { m_workDir = workDir; }
	...
	*/
	int loadCornerData(string filename, CornerDatas& cornerDatas);
	...
	*/
	int saveCornerData(string filename, const CornerDatas& cornerDatas);
	...
	*/
	int loadCameraParams(string filename, CameraParams& cameraParams);
	...
	* 参数 : filename		[in]	参数文件路径/文件名
	*/
	int saveCameraParams(const CameraParams& cameraParams, string filename = "cameraParams.yml");
	...
	*/
	int saveCalibrationDatas(string filename, RECTIFYMETHOD method, CornerDatas& cornerDatas, StereoParams& stereoParams, RemapMatrixs& remapMatrixs);
